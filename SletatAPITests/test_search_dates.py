...
class TestRequests:
    def test_compare(self):
        ...
        result_2 = req_2.json()
        dates_result = result_2["GetToursResult"]["Data"]["aaData"]
        if len(dates_result) != 0:
            for dates_info in dates_result:
                date_from_request = time.strptime(dates_result[0][dates_num].replace('.', '/'), "%d/%m/%Y")
                assert date_from_request <= time.strptime(settings.date_to, "%d/%m/%Y") and date_from_request >= time.strptime(settings.date_from, "%d/%m/%Y"), f"Запрос: {url_2} \nОтвет: {result_2}"
        else:
            print('\nВ ответ на запрос пришел пустой массив')
        print(f'visibleOperators: {settings.visibleOperators} country_id:{countryId} city_from_id:{cityFromId} date: {settings.date_from} - {settings.date_to}')
