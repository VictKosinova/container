...
def request_hotels_all():
    ...
    if len(aa_data) == 0:
        ...
        hotel_ids = [random.choices(hotel_ids_list, k=3), random.choices(hotel_ids_list, k=3),
                     random.choices(hotel_ids_list, k=3)]
        return hotel_ids
...
class TestRequests:
    def test_compare(self, hotel_id):
        _hotel_id_num = 3
        time.sleep(60)
        url_2 = f"https://module.sletat.ru/slt/Main.svc/GetTours?requestId=0&pageSize=20&pageNumber=1&countryId={countryId}&cityFromId={cityFromId}&hotels={str(hotel_id)[1:-1]}&s_nightsMin=6&s_nightsMax=6&currencyAlias=RUB&includeDescriptions=1&includeOilTaxesAndVisa=1&minHotelRating=0&s_showcase=false&filterToursForType=0&s_hotelIsNotInStop=true&s_hasTickets=true&s_ticketsIncluded=false&updateResult=1&visibleOperators={settings.visibleOperators}&s_adults=2&s_departFrom={settings.date_from}&s_departTo={settings.date_to}&calcFullPrice=1&showHotelFacilities=1&requestSource=1"
        try:
        ...
        part_req = requests.get(
            F'https://module.sletat.ru/slt/Main.svc/GetTours?requestId={request_id_2}&pageSize=20&pageNumber=1&countryId={countryId}&cityFromId={cityFromId}&hotels={str(hotel_id)[1:-1]}&s_nightsMin=6&s_nightsMax=6&currencyAlias=RUB&includeDescriptions=1&includeOilTaxesAndVisa=1&minHotelRating=0&s_showcase=false&filterToursForType=0&s_hotelIsNotInStop=true&s_hasTickets=true&s_ticketsIncluded=false&updateResult=1&visibleOperators={settings.visibleOperators}&s_adults=2&s_departFrom={settings.date_from}&s_departTo={settings.date_to}&calcFullPrice=1&showHotelFacilities=1&requestSource=1',
            headers=settings.headers, proxies=settings.proxies, verify=False)
        ...
        if len(hotels_result) != 0:
            for hotel_info in hotels_result:
                assert hotels_result[0][self._hotel_id_num] in hotel_id, f"Запрос: {url_2} \nОтвет: {result_3}"
        else:
            print('В ответ на запрос пришел пустой массив')
        print(f'visibleOperators: {settings.visibleOperators} country_id:{countryId} city_from_id:{cityFromId} hotelIds:{str(hotel_id)[1:-1]}')